...
std::string full_path_to_stl_file_wall = "./input/aorticvalve.stl";
std::string full_path_to_stl_file_fluid = "./input/vesselblood.stl";
std::string full_path_to_stl_file_constrain = "./input/constrain.stl";
...
Real Youngs_modulus = 3.3e6;

Real resolution_ref = 0.00005;

//----------------------------------------------------------------------
//	Geometry parameters for shell.
//----------------------------------------------------------------------

BoundingBox system_domain_bounds(Vec3d(-0.025, 0.025, -0.025), Vec3d(0.025, 0.137, 0.025));
// 定义buffer几何参数
Vecd buffer_halfsize_inlet = Vecd(fluid_radius_inlet, resolution_ref * 2.5, fluid_radius_inlet);
Vecd buffer_translation_inlet = Vecd(0.0, 0.025 + resolution_ref * 2.5, 0.0);
...
Vecd buffer_translation_outlet = Vecd(0, 0.137 - resolution_ref * 2.5, 0);
...
struct InflowVelocity
{
    ...
    Vecd operator()(Vecd &position, Vecd &velocity)
    {
        ...
        target_velocity[1] =
            ((10815643.4252119 * pow(time_in_period, 15) +
...
int main(int ac, char *av[])
{
    ...
    SolidBody wall_boundary(sph_system, makeShared<ConstrainBlock>("InsertedBody"));
