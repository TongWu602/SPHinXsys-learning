...
const Real rho0_f = 1060.0; /**< Reference density of fluid. */
...
const Real rho0_s = 1000.0;     /**< Reference density.*/
...
void poiseuille_flow(const Real resolution_ref, const Real resolution_shell, const Real shell_thickness)
{
    ...
    const Vec3d translation_constrain1 (0., full_length * 0.1, 0.);
    const Vec3d translation_constrain2 (0., full_length * 0.9, 0.);
    ...
    //----------------------------------------------------------------------
    const BoundingBox system_domain_bounds(Vec3d(-diameter, 0, -diameter),Vec3d(diameter, full_length, diameter));
    //----------------------------------------------------------------------
    //  Define water shape
    //----------------------------------------------------------------------
    ...
    water_block_shape->add<TriangleMeshShapeCylinder>(SimTK::UnitVec3(0., 1., 0.), fluid_radius,
                                                      ...
                                                      translation_fluid);
    //----------------------------------------------------------------------
    //  Define constrain shape×Ô¼ºÐ´µÄ
    //----------------------------------------------------------------------
    
    auto constrain_block_shape = makeShared<ComplexShape>("ConstrainBody");
    constrain_block_shape->add<TriangleMeshShapeCylinder>(SimTK::UnitVec3(0., 1., 0.), fluid_radius*2,
                                                      full_length * 0.1, SimTK_resolution,
                                                           translation_constrain1);
    
    constrain_block_shape->add<TriangleMeshShapeCylinder>(SimTK::UnitVec3(0., 1., 0.), fluid_radius * 2,
                                                           full_length * 0.9, SimTK_resolution,
                                                           translation_constrain2);
    // 
    //----------------------------------------------------------------------
    ...
    ContactRelation water_block_contact(water_block, {&shell_boundary});
    ContactRelation shell_boundary_contact(shell_boundary, {&water_block});
    //----------------------------------------------------------------------
    ...
    SimpleDynamics<thin_structure_dynamics::AverageShellCurvature> shell_curvature(water_block_contact);
    //----------------------------------------------------------------------
    //	Algorithms of fluid dynamics.
    //----------------------------------------------------------------------
    InteractionDynamics<NablaWVComplex> kernel_summation(water_block_inner, water_block_contact);
    SimpleDynamics<NormalDirectionFromBodyShape> wall_boundary_normal_direction(shell_boundary);
