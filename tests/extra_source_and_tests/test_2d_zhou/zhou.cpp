 ...
 */
Real DL = 0.2;                                             /**< Channel length. */
Real DH = 0.05;                                             /**< Channel height. */

Real a = 0.08; //长半轴
Real b = 0.03; //短半轴
Real N = 360; // 椭圆精度
//Vec2d insert_circle_center = Vec2d(0.5 * DL, 0.5 * DH);
...
BoundingBox system_domain_bounds(Vec2d(-2 * a, -2 * b), Vec2d(2 * a, 2 * b));
/**
...
Real c_f = 10.0 * U_f;
/**
 * @brief buffer parameters. 缓冲区参数设置
 */
...
class WaterBlock : public MultiPolygonShape
{
  public:
    explicit WaterBlock(const std::string &shape_name) : MultiPolygonShape(shape_name)
    {
        ...
        //定义椭圆管
        std::vector<Vecd> water_block_shape_ellipse;
        for (int i = 0; i < N + 1; ++i)
        {
            Real theta = 2.0 * Pi * Real(i) / Real(N);
            Real x = 0.5 * DL + a * cos(theta);
            Real y = 0.5 * DH + b * sin(theta);
            water_block_shape_ellipse.push_back(Vecd(x, y));
        }
        multi_polygon_.addAPolygon(water_block_shape_ellipse, ShapeBooleanOps::add);
...
class WallBoundary : public MultiPolygonShape
{
  public:
    explicit WallBoundary(const std::string &shape_name) : MultiPolygonShape(shape_name)
    {
        ...
        // 椭圆形管道
        std::vector<Vecd> outer_wall_shape_ellipse;
        for (int i = 0; i < N + 1; ++i)
        {
            Real theta = 2.0 * Pi * Real(i) / Real(N);
            Real x = 0.5 * DL + (a + BW) * cos(theta);
            Real y = 0.5 * DH + (b + BW) * sin(theta);
            outer_wall_shape_ellipse.push_back(Vecd(x, y));
        }

        std::vector<Vecd> inner_wall_shape_ellipse;
        for (int i = 0; i < N + 1; ++i)
        {
            Real theta = 2.0 * Pi * Real(i) / Real(N);
            Real x = 0.5 * DL + a * cos(theta);
            Real y = 0.5 * DH + b * sin(theta);
            inner_wall_shape_ellipse.push_back(Vecd(x, y));    
        }

        multi_polygon_.addAPolygon(outer_wall_shape, ShapeBooleanOps::add);
        multi_polygon_.addAPolygon(outer_wall_shape_ellipse, ShapeBooleanOps::add);
        multi_polygon_.addAPolygon(inner_wall_shape_ellipse, ShapeBooleanOps::sub);
